...
def select_next_date(driver):
    element = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "#cphContents_cphContents_cphContents_btnNextDate"))
    )
    element.click()
...
def main():
    try:
        ...
        target_date = "2024.08.18"  # 종료 기준 날짜

        while True:
            # 팀 순위 데이터를 추출
            if headers is None:  # 처음에만 헤더를 저장
                headers, data = extract_team_ranks(driver)
            else:
                _, data = extract_team_ranks(driver)

            # 수집한 데이터를 전체 데이터 리스트에 추가
            all_data.extend(data)

            # 현재 날짜를 추출
            current_date = driver.find_element(By.CSS_SELECTOR, "#cphContents_cphContents_cphContents_lblSearchDateTitle").text.strip()
            ...
            # 다음 날짜로 이동
            select_next_date(driver)
            
            time.sleep(random.uniform(4, 7))  # 이동 후 대기

        # 데이터 저장
        filename = '2024_일자별순위.csv'
        save_to_csv(filename, headers=headers, data=all_data)
